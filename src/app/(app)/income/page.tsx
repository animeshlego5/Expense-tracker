...
export default async function IncomePage({
  ...
  const rows = await db
    ...
    .orderBy(desc(incomes.occurredOn), desc(incomes.createdAt));
    ...
    <div className="flex flex-col gap-5">
      <header className="flex items-center justify-between">
        ...
        <nav className="flex items-center gap-1">
          <Link
            href={`/income?month=${addMonths(month, -1)}`}
      ...
      {editRow ? (
        ...
        <IncomeForm key="add" today={today} />
      )}

      {groups.length === 0 ? (
        <p className="text-ink-soft">No income in {monthLabel(month)}.</p>
      ) : (
        <div className="flex flex-col gap-5">
          {groups.map((g) => {
                  ...
                  {g.rows.map((r) => (
                    <li key={r.id} className="flex items-center gap-3 px-4 py-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-ink">{r.source}</p>
                      </div>
                      <span className="shrink-0 tabular-nums text-ink">
                        {formatPaise(r.amountPaise)}
                      </span>
                      <Link
                        href={`/income?month=${month}&edit=${r.id}`}
                        className="shrink-0 text-sm text-ink-soft underline"
                      >
                        Edit
                      </Link>
                      <form action={deleteIncome} className="shrink-0">
